...
export default function OverviewPage() {
  ...
  const { data: sentLogs, isLoading: loadingSent } = useLogs({
    limit: 1,
    status: "sent",
  });
  const { data: failedLogs, isLoading: loadingFailed } = useLogs({
    limit: 1,
    status: "failed",
  });
  const { data: queuedLogs, isLoading: loadingQueued } = useLogs({
    limit: 1,
    status: "queued",
  });
    ...
    <div className="animate-fade-in space-y-s-40">
      <div>
        ...
        <p className="text-text-secondary text-s-16 max-w-s-600">
          Monitor your email infrastructure performance and delivery stats in
          real-time.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-s-20">
        {statsConfig.map(({ label, value, icon: Icon, variant }) => (
          <Card
            ...
            hoverable
            padded={false}
            className="p-s-24 flex flex-col justify-between group overflow-hidden relative"
          >
            <div
              className={`absolute top-0 right-0 w-s-64 h-s-64 opacity-[0.03] group-hover:opacity-[0.08] transition-opacity -mr-s-16 -mt-s-16`}
            >
            ...
            <div className="flex items-center gap-s-10 mb-s-20">
              ...
              <span className="text-s-12 font-bold text-text-muted uppercase tracking-widest">
                {label}
              </span>
            </div>
            ...
            <div className="flex items-baseline gap-s-8">
              <div
                className={`text-s-32 font-black ${loading ? "text-text-muted" : "text-text-primary"}`}
              >
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-s-24">
        ...
        <Card className="flex flex-col items-center justify-center text-center">
          ...
          <h3 className="text-s-18 font-bold text-text-primary mb-s-8">
            Ready to Scale
          </h3>
          <p className="text-text-secondary text-s-14 max-w-s-300">
            Once you start sending, delivery performance charts will appear
            here.
          </p>
          <Button
            variant="primary"
