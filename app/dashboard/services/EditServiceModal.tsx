...
export function EditServiceModal({ isOpen, onClose, service, onUpdate }: EditServiceModalProps) {
  ...
  const providerName = providerDef?.name || service.provider.charAt(0).toUpperCase() + service.provider.slice(1);
  const categoryText = providerDef?.category === 'transactional' ? 'Transactional Service' : 'Personal Service';
  // Mocking the limit per day, for this showcase we follow the design which shows 500 for Gmail
  const limitText = service.provider === 'gmail' ? '500 emails per day' : 'Limit depends on provider';
        ...
        <div className="flex flex-col flex-1 overflow-y-auto custom-scrollbar bg-bg-base text-text-primary">
          ...
          <div className="flex items-center gap-s-16 p-s-24 border-b border-border-dim">
            ...
            <div className="flex flex-col gap-s-4">
              <span className="text-s-16 font-bold text-text-primary">{providerName}</span>
              <div className="flex items-center gap-s-8 text-s-12 text-text-secondary">
                ...
                <span className="flex items-center gap-s-4">
                  {limitText}
                  <HelpCircle size={12} className="cursor-help transition-opacity opacity-70 hover:opacity-100" />
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="p-s-24 flex flex-col gap-s-24">
            ...
            <div>
               ...
               <div className="relative">
                 ...
                 />
                 <button className="absolute right-s-12 top-1/2 -translate-y-1/2 text-text-muted hover:text-text-secondary transition-colors cursor-pointer p-s-4">
                   <Copy size={16} />
                 </button>
               </div>
            </div>
            ...
            <div className="flex flex-col gap-s-8">
               ...
               <div className="flex items-center justify-between p-s-16 border border-border rounded-s-12 bg-bg-card">
                 <span className="text-s-14 text-text-primary">
                   Connected as <span className="font-semibold text-accent">jasleendashboard@gmail.com</span>
                 </span>
