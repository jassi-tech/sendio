  ...
  const ServiceCard = ({ service }: { service: ServiceDef }) => (
    <div 
      className={`relative flex flex-col items-center justify-center p-s-16 bg-bg-card border border-border rounded-s-8 transition-all overflow-hidden ${
        service.isSelectable ? 'cursor-pointer hover:border-accent hover:bg-bg-hover hover:-translate-y-s-2 hover:shadow-accent-glow group' : 'cursor-not-allowed'
      }`}
      onClick={() => {
    ...
    >
      <div className={`w-s-40 h-s-40 mb-s-8 flex items-center justify-center bg-white rounded-s-8 p-s-4 shadow-[0_0_10px_rgba(0,0,0,0.1)] transition-transform ${service.isSelectable ? 'group-hover:scale-110' : 'opacity-50 grayscale'}`}>
        {service.id === 'smtp' ? (
        ...
        <div className="p-s-24 overflow-y-auto flex-1 custom-scrollbar">
          {loading ? (
            <div className="flex justify-center flex-col items-center gap-s-16 p-s-48">
              ...
              <span className="text-text-secondary text-s-14">Loading services...</span>
            </div>
          ) : (
            <div className="flex flex-col gap-s-32">
              ...
              <div className="flex flex-col gap-s-16">
                <div className="flex items-center gap-s-8 text-text-secondary">
                  <h3 className="text-s-14 font-semibold text-text-primary">Personal Services</h3>
              ...
              <div className="flex flex-col gap-s-16">
                <div className="flex items-center gap-s-8 text-text-secondary">
                  <h3 className="text-s-14 font-semibold text-text-primary">Transactional Services</h3>
                  <HelpCircle size={14} className="opacity-70 cursor-help hover:opacity-100 transition-opacity" />
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-s-12">
