...
export default function TemplatesPage() {
  ...
  const { data: templates = [], isLoading: loadingList } = useTemplates();
  const createMutation = useCreateTemplate();
  const updateMutation = useUpdateTemplate(editing?.templateId || "");
  const deleteMutation = useDeleteTemplate();
  const queryClient = useQueryClient();

  const loading = loadingList || fetchingFull;
  ...
  const openEdit = async (t: Template) => {
    try {
      ...
      const full = await queryClient.fetchQuery({
        queryKey: ["template", t.templateId],
        queryFn: () => templatesApi.get(t.templateId),
      }) as FullTemplate;
      setEditing(full);
      setForm({ name: full.name, subject: full.subject, html: full.html });
      setShowForm(true);
    } catch (err: any) {
      showToast("Failed to load full template", "error");
    } finally {
      ...
      <div className="animate-fade-in space-y-s-24">
        <div className="flex items-center gap-s-16">
          ...
          <div>
            ...
            <p className="text-s-13 text-text-muted font-medium">
              Crafting: {form.name || "Untitled Template"}
            </p>
          </div>
        </div>
        ...
        <div className="bg-bg-card border border-border rounded-s-24 shadow-2xl flex flex-col overflow-hidden min-h-[800px]">
          <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
            ...
            <form
              id="template-form"
              onSubmit={handleSave}
              className="flex-1 flex flex-col overflow-hidden border-r border-border border-b lg:border-b-0 w-full lg:w-1/2"
            >
              ...
              <div className="flex-1 flex flex-col bg-bg-base/50 p-s-24">
                <div className="flex items-center justify-between mb-s-12">
                  <label className="text-s-13 font-bold text-text-secondary uppercase tracking-wider">
                    HTML Canvas
                  </label>
                  <Badge
                    variant="neutral"
                    className="font-mono text-[10px] bg-white/5 border-white/10 text-text-muted"
                  >
                ...
                <div className="flex-1 rounded-s-16 border border-white/10 bg-[#1e1e1e] p-[1px] focus-within:border-accent/50 transition-colors shadow-inner overflow-hidden flex flex-col min-h-[400px]">
                  <div className="flex items-center px-s-16 py-s-10 border-b border-white/10 bg-[#2d2d2d]">
                    ...
                    <span className="ml-s-12 text-[#858585] text-s-12 font-mono">
                      template.html
                    </span>
                  </div>
                  <textarea
                ...
                <div className="min-h-full p-s-32 flex justify-center items-start">
                  <div className="w-full max-w-[600px] bg-white shadow-2xl rounded-sm border border-gray-200 overflow-hidden min-h-[500px] flex flex-col">
                    ...
                    <div className="p-s-16 border-b border-gray-200 bg-gray-50/80 space-y-s-8 text-s-13 font-sans">
                      <div className="flex">
                        <span className="w-s-64 text-gray-400 font-medium text-right pr-s-12">
                          From:
                        </span>
                    ...
                    <div className="flex-1 relative min-h-[400px]">
                      {form.html ? (
                        ...
                        <div className="absolute inset-0 flex items-center justify-center text-gray-300 flex-col gap-s-16">
                          ...
                          <span className="text-s-14 font-medium uppercase tracking-widest text-center text-gray-400">
                            Start coding
                            <br />
                            to see preview
                          </span>
          ...
          <div className="p-s-24 border-t border-border flex flex-col sm:flex-row items-center justify-between bg-bg-elevated/50 z-20 gap-s-16">
            ...
            <div className="flex gap-s-16 w-full sm:w-auto">
              ...
              <Button
                ...
                loading={saving}
                icon={<Save size={18} />}
                className="flex-1 sm:flex-none px-s-32"
              >
    ...
    <div className="animate-fade-in space-y-s-32">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-s-16">
        <div>
          ...
          <p className="text-text-secondary text-s-15">
            Customizable HTML layouts for automated communication.
          </p>
        </div>
      ...
      {loading ? (
        <div className="p-s-80 flex flex-col items-center justify-center text-text-muted gap-s-16">
          ...
          <span className="text-s-14 font-medium tracking-wide">
            Gathering templates...
          </span>
        </div>
      ) : templates.length === 0 ? (
        <Card
          padded
          variant="elevated"
          className="flex flex-col items-center justify-center text-center p-s-80 space-y-s-24"
        >
          ...
          <div>
            <h3 className="text-s-20 font-bold text-text-primary">
              No Canvas Yet
            </h3>
            <p className="text-text-secondary text-s-15 max-w-s-400 mx-auto leading-relaxed">
              Email templates help you maintain consistent branding and dynamic
              content across all your communications.
            </p>
          </div>
          <Button
            onClick={openNew}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-s-24">
          {templates.map((t) => (
            <Card
              key={t.templateId}
              padded
              className="group transition-all hover:border-accent hover:-translate-y-s-4 flex flex-col relative overflow-hidden"
            >
              ...
              <div className="flex items-start justify-between mb-s-20">
                ...
                <div className="flex gap-s-8">
                  <Button
                    ...
                    onClick={() => openEdit(t)}
                    icon={<Edit3 size={18} />}
                    className="hover:bg-accent/10 hover:text-accent rounded-full w-s-36 h-s-36 p-0"
                  />
                  {!t.isDefault && (
                    <Button
                      variant="ghost"
                      ...
                      icon={<Trash2 size={18} />}
                      className="hover:text-error hover:bg-error/5 rounded-full w-s-36 h-s-36 p-0"
                    />
                  )}
                </div>
              </div>
              <div className="flex-1">
                <h3 className="text-s-18 font-bold text-text-primary mb-s-8 group-hover:text-accent transition-colors line-clamp-1">
                  {t.name}
                </h3>
                <p className="text-s-14 text-text-secondary mb-s-20 line-clamp-2 leading-relaxed">
                  <span className="text-text-muted italic mr-s-4">
                    Subject:
                  </span>{" "}
                  {t.subject}
                </p>
                  ...
                  <div className="flex gap-s-8 flex-wrap mb-s-20">
                    {t.variables.map((v) => (
                      <Badge
                        key={v}
                        variant="neutral"
                        className="text-[10px] font-mono lowercase tracking-normal bg-bg-elevated/80 py-s-2 px-s-8 border-border/50"
                      >
                        {v}
                      </Badge>
                    ))}
              ...
              <div className="pt-s-20 border-t border-border mt-auto flex items-center justify-between">
                ...
                >
                  {copiedId === t.templateId ? (
                    ...
                    <Copy
                      size={12}
                      className="group-hover/id:scale-110 transition-transform"
                    />
                  )}
                  <span className="opacity-80 font-medium">
                    {t.templateId || "Generating..."}
                  </span>
                </div>
                <Badge
                  variant={t.isDefault ? "info" : "neutral"}
                  className="px-s-8 py-s-2 text-[10px] font-semibold uppercase tracking-wider"
                >
