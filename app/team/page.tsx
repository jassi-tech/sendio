...
const COMPANY_VALUES = [
  {
    ...
    description: 'We build for developers, with developers. Every feature is designed with the developer experience in mind.',
    icon: '💻',
  },
  {
    title: 'Security & Privacy',
    description: 'Your data is sacred. We implement industry-leading security practices and are transparent about how we protect your information.',
    icon: '🔒',
  },
  {
    title: 'Transparency',
    description: 'We believe in open communication. You always know what\'s happening with your emails and your data.',
    icon: '🤝',
  },
  {
    title: 'Reliability',
    description: 'Email delivery is critical. We maintain 99.99% uptime and redundant infrastructure for your peace of mind.',
    icon: '⚡',
  },
];
    ...
    <div className="min-h-screen bg-bg-base text-text-primary">
      ...
      <section className="pt-s-120 pb-s-80 px-s-40">
        <div className="max-w-s-1000 mx-auto text-center">
          ...
          <p className="text-s-18 text-text-secondary max-w-s-700 mx-auto leading-relaxed">
            We're a team of passionate developers and builders dedicated to making email delivery simple, secure, and reliable.
          </p>
        </div>
      </section>
      ...
      <section className="py-s-100 px-s-40 bg-bg-card/30">
        <div className="max-w-s-1200 mx-auto">
          <h2 className="text-s-40 font-bold text-center mb-s-64">Our Values</h2>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-s-32">
            {COMPANY_VALUES.map((value, idx) => (
              <div key={idx} className="card p-s-40 hover:border-text-muted/50 transition-all">
                <div className="text-s-56 mb-s-24">{value.icon}</div>
                <h3 className="text-s-24 font-bold mb-s-16">{value.title}</h3>
                <p className="text-s-14 text-text-secondary leading-relaxed">{value.description}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-s-100 px-s-40">
        <div className="max-w-s-1400 mx-auto">
          <h2 className="text-s-40 font-bold text-center mb-s-64">Leadership Team</h2>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-s-32">
            {TEAM_MEMBERS.map((member) => (
              <div key={member.id} className="card p-s-40 hover:border-text-muted/50 transition-all group">
                {/* Avatar */}
                ...
                <p className="text-s-14 text-accent font-bold mb-s-12">{member.role}</p>
                <p className="text-s-13 text-text-secondary leading-relaxed mb-s-24">{member.bio}</p>

                {/* Social Links */}
                <div className="flex items-center gap-s-12">
                  {member.social.email && (
                    <a href={`mailto:${member.social.email}`} className="p-s-8 bg-bg-card rounded-s-6 text-text-secondary hover:text-accent transition-colors">
                      <Mail className="w-s-16 h-s-16" />
                    </a>
                  )}
                  {member.social.linkedin && (
                    <a href={member.social.linkedin} className="p-s-8 bg-bg-card rounded-s-6 text-text-secondary hover:text-accent transition-colors">
                      <Linkedin className="w-s-16 h-s-16" />
                    </a>
                  )}
                  {member.social.github && (
                    <a href={member.social.github} className="p-s-8 bg-bg-card rounded-s-6 text-text-secondary hover:text-accent transition-colors">
                      <Github className="w-s-16 h-s-16" />
                    </a>
                  )}
                  {member.social.twitter && (
                    <a href={member.social.twitter} className="p-s-8 bg-bg-card rounded-s-6 text-text-secondary hover:text-accent transition-colors">
      ...
      <section className="py-s-100 px-s-40 bg-bg-card/30">
        <div className="max-w-s-1000 mx-auto">
          ...
          <div className="space-y-s-24 text-s-16 text-text-secondary leading-relaxed">
            <p>
              MailFlow was founded in 2023 by a group of engineers who were frustrated with existing email delivery platforms. 
              They wanted a service that gave developers complete control, transparency, and the ability to use their own infrastructure.
            </p>
            <p>
              Today, MailFlow powers transactional emails for thousands of developers and companies worldwide. We're backed by 
              leading venture capital firms and are on a mission to make email delivery simple, secure, and developer-friendly.
            </p>
            <p>
              Our team is distributed across multiple continents, with offices in San Francisco, Berlin, and Singapore. 
              We're constantly hiring talented engineers, product managers, and customer success specialists. If you're interested 
              in joining us, check out our careers page!
            </p>
          </div>
          ...
          <div className="mt-s-64 grid grid-cols-1 md:grid-cols-3 gap-s-24">
            <div className="card p-s-32 text-center">
              <div className="text-s-48 font-black text-accent mb-s-12">50+</div>
              <p className="text-s-14 text-text-secondary">Team Members</p>
            </div>
            <div className="card p-s-32 text-center">
              <div className="text-s-48 font-black text-success mb-s-12">5K+</div>
        ...
        <div className="max-w-s-900 mx-auto">
          <div className="card glass p-s-64 text-center">
            ...
            <p className="text-s-16 text-text-secondary mb-s-48 leading-relaxed">
              We're hiring talented engineers and product enthusiasts. Help us build the future of email delivery.
            </p>
            <a href="#" className="inline-block px-s-40 py-s-20 bg-accent text-white rounded-s-12 font-bold text-s-16 transition-all hover:scale-105 hover:shadow-accent-glow">
              View Open Positions
            </a>
          </div>
        </div>
